...
class Watcher(tweepy.StreamListener):
    ...
    def search_extended_hashtags(self, status):
        ...
        if not hasattr(status, "extended_tweet"):
            return False
        count = status.extended_tweet['full_text'].count("#")
        if count > 4:
            return True
        else:
            return False
    ...
    def on_error(self, error):
        ...
        # This error-handling was more complicated, but it keeps breaking in new ways without changes occurring. For now, it's just logging the behaviour.
